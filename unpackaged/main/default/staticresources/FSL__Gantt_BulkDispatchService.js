...
(function () {
    ...
    bulkDispatchService.$inject = ['$compile', '$rootScope', 'ResourcesAndTerritoriesService', 'ServiceSelectorService', 'userSettingsManager', 'StateService', 'servicesService', 'TimePhasedDataService', 'SERVICE_STATUS'];

    angular.module('serviceExpert').factory('bulkDispatchService', bulkDispatchService);

    function bulkDispatchService($compile, $rootScope, ResourcesAndTerritoriesService, ServiceSelectorService, userSettingsManager, StateService, servicesService, TimePhasedDataService, SERVICE_STATUS) {
        ...
        function open() {
            ...
            $scope.bulkDispatch = bulkDispatch;
            $scope.results = {};
            ...
            $scope.selectedMashu = customLabels.x_selected;
        ...
        function bulkDispatch() {
            ...
            servicesService.changeStatus(ids, SERVICE_STATUS.DISPATCHED, startDate, finishDate, true).then(function (resultObjects) {
                ...
                for (var i = 0; i < ids.length; i++) {
                    if (TimePhasedDataService.serviceAppointments()[ids[i]].status === SERVICE_STATUS.DISPATCHED) $scope.results.dispatched.push(TimePhasedDataService.serviceAppointments()[ids[i]]);
                }

                $scope.results.failed = Object.keys(resultObjects.failedResults).length > 0 ? resultObjects.failedResults : null;
            }).catch(function (err) {
                console.warn('bulk dispatch failed :(');
                console.log(err);
            }).finally(function () {
