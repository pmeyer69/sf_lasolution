...
function GanttController($rootScope, $scope, DataService, UtilsService, GeneralLightbox, ScmLightbox, TerritoryFilteringService, GetCrewCandidates, CreateCrewLightbox, $timeout) {
    ...
    function createGanttTimeline() {
        ...
        for (var territoryId in territories) {
            ...
            var _loop = function _loop(id) {
                ...
                if (!stm.ServiceResource || !stm.ServiceResource.ServiceCrewId) {
                    return 'continue';
                }
                ...
                for (var resourceId in resources) {
                    ...
                    for (var id in currentResource.crewMemberships) {
                        ...
                        crewIdsToChildrenMap[scm.ServiceCrewId].push({
                            ...
                            key: scm.ServiceResourceId + '_' + territoryId + '_' + scm.ServiceCrewId
                        });
    ...
    function prepareDatesBeforeFetching() {
        ...
        // CFSL745
        if (scheduler._max_date) {
            dates.end.setDate(3 + dates.end.getDate());
        } else {
            dates.end.setDate(8 + dates.end.getDate());
        }

        dates.start.setDate(dates.start.getDate() - 3);
...
function CrewManagementTerritory(ter, timezone) {
    ...
    this.label = ter.Name.encodeHTML() + '<div class="cm-timezone-gantt">' + timezone + ' (GMT' + gmtSign + window.Math.abs(timezoneDiffInHours) + ')</div>';
...
function CreateCrewLightbox($rootScope, $compile, DataService, UtilsService, RemoteActionsService) {
    ...
    var crewColors = ['#e5410f', '#f8611d', '#ffc455', '#facb00', '#dcda5c', '#7ac47a', '#23ca54', '#0faa6f', '#9c61b4', '#a37fdb', '#f87393', '#eb686b', '#f16d60', '#ed6e91', '#d54409', '#0077be', '#3695e0', '#5eb6e7', '#61cae6'];
    ...
    function init() {
        ...
        scmEnd.setHours(scmEnd.getHours() + 24);
    ...
    function createCrew() {
        ...
        if ($scope.crewObject.stmEnd) {
            $scope.crewObject.stmEnd = UtilsService.convertDateToUTC($scope.crewObject.stmEnd, $scope.territories[$scope.crewObject.stmTerritoryId].timezone);
        }

        if ($scope.crewObject.scmEnd) {
            $scope.crewObject.scmEnd = UtilsService.convertDateToUTC($scope.crewObject.scmEnd, $scope.territories[$scope.crewObject.stmTerritoryId].timezone);
        }
    ...
    function formatDate(type) {
        ...
        if ($scope.crewObject[type]) {
            return moment($scope.crewObject[type]).format('ll');
        }
    ...
    function nextStep() {
        ...
        if ($scope.step === 3 && !$scope.resouceHasOverlappingScm) {
            !justCheck && $scope.step++;
            return true;
        }
...
function DataService($q, RemoteActionsService, UtilsService, $rootScope) {
    ...
    function getGanttData(requestedStartDate, requestedEndDate) {
        ...
        RemoteActionsService.callRemoteAction('getGanttData', requestedStartDate, requestedEndDate, window.__crews.loadedTerritories || []).then(function (data) {
            ...
            for (var key in crews) {
                ...
                for (var id in resource.territoryMemberships) {
                    ...
                    if (resourceTypeIsCrew) {
                        ...
                        ganttAbsence.resourceId = ganttAbsence.ResourceId + '_' + stm.ServiceTerritoryId + '_' + crews[key].ServiceCrewId;
                    }
...
function TerritoryFilteringService($compile, $rootScope, DataService, RemoteActionsService, UtilsService) {
    ...
    DataService.getResourcesPromise().promise.then(function () {
        ...
        window.__crews.loadedTerritories.forEach(function (territoryId) {
...
function UtilsService() {
    ...
    function generateResourceGanttLabel(resource, allSkills) {
        ...
        if (resource[ganttLabelField]) {
            label += '<div class="cm-resource-name-gantt truncate">' + resource.Name.encodeHTML() + '</div>';
            label += '<div class="cm-resource-gantt-label truncate">' + resource[ganttLabelField].encodeHTML() + '</div>';
        } else {
            label += '<div class="cm-resource-name-gantt resource-gantt-no-label cm-resource-gantt-label truncate">' + resource.Name.encodeHTML() + '</div>';
        }
