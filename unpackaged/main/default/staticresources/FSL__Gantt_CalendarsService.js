		...
		function drawByCalendarAndTime(start, finish, calendarId, sectionId, isSecondary, calObj, shiftsByResource) {
			...
			for (var j = 0; j < daysOfTheWeek.length; j++) {
					...
					if (todaysShifts) {
						for (var _i2 = 0; _i2 < todaysShifts.length; _i2++) {
							if (todaysShifts[_i2].serviceTerritoryId == sectionId.split('_')[1] || !todaysShifts[_i2].serviceTerritoryId) {
								slotsAndShifts.push({ slotStart: todaysShifts[_i2].startTime, slotFinish: todaysShifts[_i2].endTime, type: todaysShifts[_i2].timeSlotType, record: 'shift', hasTerritory: !!todaysShifts[_i2].serviceTerritoryId });
							}
						}
					}
				}
				...
				for (var _i4 = 0; _i4 < slotsCombined.length; _i4++) {
					...
					if (slotsCombined[_i4].type == 'Extended') {
					...
					// in case we want to differ shifts from slots
					// else if (slotsCombined[i].record == 'shift') {
					// 	addMarkedTimeSpan(lastInvalidDate, slotStart, sectionId, null);
					// 	addMarkedTimeSpan(slotStart, slotFinish, sectionId, slotsCombined[i].record);
					// }
					else {
		...
		function combineToTypedSlots(_intervalsArray, _slotsAndShifts) {
			...
			for (var i = 0; i < _intervalsArray.length - 1; i++) {
				...
				for (var j = 0; j < _slotsAndShifts.length; j++) {
					...
					if (intersection.intersection) {
						...
						if (currentSlot === undefined || currentSlot.record == 'timeslot' || currentSlot.record == 'shift' && !currentSlot.hasTerritory && slot.hasTerritory) {
							tempInterval = {
			...
			return typedSlots.reduce(function (combined, next) {
				if (!combined.length) {
					...
					if (prev.slotFinish == next.slotStart && prev.record == next.record && prev.type == next.type) {
